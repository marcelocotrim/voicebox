...
| **Qwen TTS 0.6B 4bit** | `qwen` | `mlx-community/Qwen3-TTS-12Hz-0.6B-Base-4bit` | 1.7 GB | ~1–2 GB | 10 |
...
On Apple Silicon, Qwen TTS uses MLX-optimized repos from `mlx-community` instead of the PyTorch repos. The backend picks automatically via `get_backend_type()`. The 4bit Qwen TTS variant is MLX-only and is only registered on Apple Silicon.
